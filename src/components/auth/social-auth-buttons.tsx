...
export function SocialContinueButton({
  ...
  return (
    <button
      ...
      className={`flex w-full items-center justify-center gap-3 rounded-xl px-4 py-3.5 text-sm font-semibold transition-all disabled:cursor-not-allowed disabled:opacity-45 ${
        provider === "apple"
          ? "bg-black text-white hover:bg-neutral-900"
          : "border border-neutral-200 bg-white text-[#1a1a1a] shadow-sm hover:bg-neutral-50"
      }`}
    >
      {loading ? (
        <Loader2
          className={`size-5 animate-spin ${provider === "apple" ? "text-white" : "text-[#036AAF]"}`}
          aria-hidden
        />
      ) : provider === "google" ? (
        <GoogleLogo className="size-5" />
