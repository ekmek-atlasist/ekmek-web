...
function parseCityStorage(cityStr: string): { city: string; district: string } {
...
function resolveWorkTypeId(stored: string): string {
  if (WORK_TYPES.some((type) => type.id === stored)) {
    return stored;
  }
  const fromLabel = workTypeIdFromLabel(stored);
  return fromLabel || stored;
}

type LocationMode = "existing" | "workplace" | "current";
...
export default function IlanDuzenlePage() {
  ...
  const [existingDistrict, setExistingDistrict] = useState("");
  ...
  useEffect(() => {
    if (!isAuthReady || !listingId) return;
    ...
    async function loadListing() {
      ...
      try {
        ...
        if (ownerId !== user!.uid) {
          ...
          setTimeout(() => {
            router.replace("/isveren/panel/ilanlar");
          }, 2000);
          return;
        }

        setTitle((data.title as string | undefined) ?? "");
        setDescription((data.description as string | undefined) ?? "");

        const cityStr = (data.city as string | undefined) ?? "";
        const parsed = parseCityStorage(cityStr);
        setExistingLocation(cityStr);
        setExistingCity(parsed.city);
        setExistingDistrict(parsed.district);
        setLocationMode("existing");

        const storedWorkType = (data.workType as string | undefined) ?? "";
        setWorkType(resolveWorkTypeId(storedWorkType));
        setPositions(Array.isArray(data.positions) ? data.positions : []);
        setSelectedBenefits(Array.isArray(data.benefits) ? data.benefits : []);
        setWorkShift((data.workShift as string | undefined) ?? "");
        setIsUrgent(Boolean(data.isUrgent));
        setPrefersDisabledCandidates(Boolean(data.prefersDisabledCandidates));

        const salaryMin = (data.salaryMin as string | null | undefined) ?? null;
        const salaryMax = (data.salaryMax as string | null | undefined) ?? null;
        const indices = salaryIndicesFromStorage(salaryMin, salaryMax);

        if (indices) {
          setSkipSalary(false);
          setMinSalaryIndex(indices.minIndex);
          setMaxSalaryIndex(indices.maxIndex);
        } else {
          setSkipSalary(true);
        }
        ...
        if (!cancelled) {
          setError("İlan yüklenemedi. Lütfen tekrar dene.");
          setLoadState("not_found");
        }
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError("İlan güncellenemedi. Lütfen tekrar dene.");
      setIsSubmitting(false);
    }
  }

  if (!isAuthReady || loadState === "loading") {
    return (
    ...
    return (
      <div className="py-16 text-center">
        ...
        <Link
          ...
          className="mt-4 inline-block text-sm font-medium text-[#036AAF] hover:underline"
        >
          ← İlanlarım
        </Link>
      </div>
  ...
  if (loadState === "not_found") {
    return (
      <div className="py-16 text-center">
        <p className="text-base font-medium text-[#1a1a1a]">İlan bulunamadı</p>
        <Link
          href="/isveren/panel/ilanlar"
          className="mt-4 inline-block text-sm font-medium text-[#036AAF] hover:underline"
        >
  ...
  return (
    <div>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#1a1a1a]">İlanı Düzenle</h1>
          <p className="mt-1 text-sm text-neutral-500">
            Mevcut ilan bilgilerini güncelle
          </p>
        </div>
        <Link
          href="/isveren/panel/ilanlar"
      ...
      <form className="mt-8 space-y-10" onSubmit={handleSubmit} noValidate>
        <section>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              {existingLocation ? (
                <p className="mb-4 rounded-2xl bg-neutral-50 px-4 py-3 text-sm text-[#1a1a1a]">
                  Mevcut konum:{" "}
                  {formatLocationDisplay(existingCity, existingDistrict)}
                </p>
              ) : null}
              ...
              {locationMode === "existing" ? (
                <p className="mt-3 text-sm text-neutral-500">
                  Değiştirmek için yukarıdan bir seçenek seçin.
                </p>
              ) : locationMode === "workplace" ? (
                isLoadingProfile ? (
                  <p className="mt-3 text-sm text-neutral-500">
        ...
        {successMessage ? (
          <p
            className="rounded-2xl bg-green-50 px-5 py-3 text-sm text-green-700"
            role="status"
          >
            {successMessage}
          </p>
